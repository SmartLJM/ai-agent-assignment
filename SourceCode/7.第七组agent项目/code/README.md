...
- `PartB_智能体应用项目/code/`：本地问答、NIfTI 处理、真实标注可视化、多模态检索和工作流引擎。
- `PartB_智能体应用项目/workflows.json`：4 个预定义工作流及 Agent 角色、输入、输出和参数。
...
- 4 个 JSON 工作流模板，满足“至少 3 个”。
- `workflow_engine.py` 实际支持：串行、线程池并行、条件分支。
- 每个节点声明 Agent 角色、输入和输出；共享 context 负责数据传递。
- 每次执行保存 `node_id / agent / mode / status / elapsed_ms / input_keys / output / decision`。
- 12 道额外编排评测覆盖模板数、三模式、并行合并、双条件分支和失败处理。
- 流程图：`evaluation/flowcharts/workflow_modes.svg` 与每个工作流的 Mermaid 文件。
...
| 4 个工作流模板 | `PartB_智能体应用项目/workflows.json` |
| 串行、并行、条件分支 | `evaluation/workflow_demo_results.json` |
| 12 道编排评测 | `evaluation/orchestration_benchmark_results.json` |
...
| 可视化流程图 | `evaluation/flowcharts/workflow_modes.svg` |
