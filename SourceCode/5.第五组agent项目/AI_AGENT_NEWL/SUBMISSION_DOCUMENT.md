...
本提交将以下四部分统一整理到本说明文档，并随完整项目一起打包：

1. 修改后的完整项目：`AI_AGENT_NEWL`
2. 评分文档：`SCORING_REPORT.md`
3. 修订摘要：`REVISION_SUMMARY.md`
4. 小组分工说明：`GROUP_CONTRIBUTION.md`

其中 `SCORING_REPORT.md` 与 `GROUP_CONTRIBUTION.md` 已保留在项目根目录；`REVISION_SUMMARY.md` 的核心内容已合并进本文档。
...
| 知识问答项目 | 30 | 选择进阶动态编排 Lv.3 与多模态知识库 Lv.2，难度和 5；动态编排和多模态评测全部通过 | 30/30 |
...
本项目选择进阶动态编排 Lv.3 与多模态知识库 Lv.2，难度和为 `3 + 2 = 5`，满足课程要求。

### 模块一：进阶动态编排 Lv.3

- `agent_registry.json` 定义 Agent 能力、输入、输出、并行能力和 fallback 关系
- `DynamicPlanner` 根据用户问题实时生成工作流，而不是只选择预置模板
- `DynamicWorkflowExecutor` 执行顺序、并行和条件节点，并支持运行时插入新节点
- `QualityGate` 可触发小目标复核、大区域复核或 query rewrite retry
- `RecoveryManager` 可在 MedSAM 不可用、病例不存在等情况下切换 fallback agent
- 动态编排评测：12/12 通过；其中 4 个计划触发运行时调整，1 个计划触发重试，2 个计划触发恢复
- 决策轨迹、动态计划对照、恢复案例和流程图保存在 `PartB_智能体应用项目/evaluation/`
...
- 根据改进文档新增 Lv.3 进阶动态编排证据，包括运行时计划生成、动态节点插入、retry、fallback recovery 和可审计决策轨迹。

## 提交提醒

- PR 描述中说明选修模块为：进阶动态编排 Lv.3 + 多模态知识库 Lv.2，难度和 5。
