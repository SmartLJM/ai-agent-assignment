...
def load_optional_mask(case_id: str, mask_type: str, dataset_dir: str | Path | None = None) -> np.ndarray | None:
    """Load the ground-truth mask or an optional model prediction."""
    case_path = _case_dir(case_id, dataset_dir)
    mask_type = (mask_type or "真实 mask").strip()
    mask_type_lower = mask_type.lower()

    if "segrap" in mask_type_lower:
        candidates = [case_path / "prediction_segrap2023.npy"]
        mask_path = next((path for path in candidates if path.exists()), None)
        if mask_path is None:
            return None
    elif "medsam" in mask_type_lower:
        candidates = [
        ...
        if mask_path is None:
            return None
    elif "pred" in mask_type_lower or "model" in mask_type_lower:
        candidates = [
            case_path / "prediction_mask.npy",
            case_path / "prediction_segrap2023.npy",
            case_path / "prediction_medsam2.npy",
            case_path / "prediction_medsam.npy",
            case_path / "prediction_medsam_slice.npy",
        ]
...
def render_case(
    ...
    if selected_mask is None:
        ...
        if selected_mask.shape != gt_mask.shape:
            raise ValueError(f"所选 mask 与真实 mask 尺寸不一致: {selected_mask.shape} vs {gt_mask.shape}")
        missing_mask = False
    ...
    is_reference = "真实" in mask_type or "ground" in mask_type.lower()
    metrics = None
    if not is_reference and not missing_mask:
        metrics = {
            "volume_dice": dice_score(selected_mask, gt_mask),
            "volume_iou": iou_score(selected_mask, gt_mask),
    ...
    return {
        ...
        "mask_statistics": {
            "labels": [int(value) for value in np.unique(selected_mask)],
