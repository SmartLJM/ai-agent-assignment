...
def _vertical_flow_svg(title: str, steps: list[tuple[str, str, str]]) -> str:
    width = 1200
    height = 140 + len(steps) * 86
    parts = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">',
        '<defs><marker id="arrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L9,3 z" fill="#94a3b8"/></marker></defs>',
        '<rect width="100%" height="100%" fill="#020617"/>',
        _svg_text(width // 2, 55, title, 28, 700, "#f8fafc"),
    ]
    x = 410
    box_w = 380
    box_h = 58
    for index, (name, detail, color) in enumerate(steps):
        y = 100 + index * 86
        parts.append(_svg_box(x, y, box_w, box_h, name, detail, color))
        if index:
            parts.append(_arrow(width // 2, y - 28, width // 2, y))
    parts.append("</svg>")
    return "".join(parts)


def build_advanced_dynamic_orchestration_svg() -> str:
    return _vertical_flow_svg(
        "Advanced Dynamic Orchestration Lv.3",
        [
            ("User Query", "task request enters runtime planner", "#22d3ee"),
            ("Intent Analyzer", "detect QA / case / prediction intent", "#34d399"),
            ("Complexity Estimator", "score evidence, case, mask, model needs", "#a78bfa"),
            ("Agent Registry", "select agents by capability and fallback", "#fbbf24"),
            ("Dynamic Planner", "generate runtime workflow nodes", "#22d3ee"),
            ("Generated Workflow", "not selected from workflows.json", "#34d399"),
            ("Execution", "sequential / parallel / conditional", "#a78bfa"),
            ("Quality Gate", "inspect intermediate result quality", "#fbbf24"),
            ("Retry / Add Agent / Fallback", "runtime adjustment on failure", "#fb7185"),
            ("Decision Trace", "record observation, reason, action, output", "#22d3ee"),
        ],
    )


def build_dynamic_recovery_loop_svg() -> str:
    return _vertical_flow_svg(
        "Dynamic Recovery and Retry Loop",
        [
            ("Agent Execution", "run generated node", "#22d3ee"),
            ("Error or Low Quality?", "quality gate or exception check", "#fbbf24"),
            ("Recovery Manager", "choose retry / rewrite / fallback", "#fb7185"),
            ("Add Runtime Node", "insert recovery node into workflow", "#a78bfa"),
            ("Re-execute", "run rewritten query or fallback agent", "#34d399"),
            ("Quality Gate", "validate recovered output", "#fbbf24"),
            ("Report + Trace", "final answer with decision log", "#22d3ee"),
        ],
    )


def generate_dynamic_flowcharts(output_dir: str | Path | None = None) -> dict:
    output_dir = Path(output_dir or (EVALUATION_DIR / "flowcharts"))
    output_dir.mkdir(parents=True, exist_ok=True)
    advanced = output_dir / "advanced_dynamic_orchestration.svg"
    recovery = output_dir / "dynamic_recovery_loop.svg"
    advanced.write_text(build_advanced_dynamic_orchestration_svg(), encoding="utf-8")
    recovery.write_text(build_dynamic_recovery_loop_svg(), encoding="utf-8")
    return {
        "files": [_manifest_path(advanced), _manifest_path(recovery)],
        "dynamic_features": [
            "runtime_workflow_generation",
            "quality_gate",
            "retry_loop",
            "fallback_recovery",
            "decision_trace",
...
def generate_flowcharts(output_dir: str | Path | None = None) -> dict:
    ...
    dynamic = generate_dynamic_flowcharts(output_dir)
    files.extend(dynamic["files"])
